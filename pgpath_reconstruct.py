...
def load_prediction_confidence(
    confidence_csv_path: Path,
    target_sample: str,
    threshold: float,
) -> Set[str]:
    """
    Load branch prediction confidence and retain nodes above threshold.
    """
    ...
    if target_sample not in confidence_df.index:
        raise ValueError(
            f"Target sample '{target_sample}' was not found in confidence file."
        )

    confidence_row = confidence_df.loc[target_sample]

    retained_branches = {
        str(branch)
        for branch, confidence in confidence_row.items()
        if float(confidence) >= threshold
    }

    return retained_branches


def filter_predicted_nodes_by_branches(
    pred_csv_path: Path,
    target_sample: str,
    retained_branches: Set[str],
) -> Set[str]:
    """Map retained confidence columns to their predicted GFA node IDs."""
    preds_df = pd.read_csv(pred_csv_path, index_col=0)
    prediction_row = preds_df.loc[target_sample]
    prediction_by_branch = {
        str(branch): value
        for branch, value in prediction_row.items()
    }

    missing_branches = retained_branches.difference(prediction_by_branch)
    if missing_branches:
        preview = ", ".join(sorted(missing_branches)[:10])
        raise ValueError(
            "Confidence file contains retained branches that are missing from "
            f"the prediction CSV: {preview}"
        )

    return {
        str(prediction_by_branch[branch])
        for branch in retained_branches
        if pd.notna(prediction_by_branch[branch])
        and str(prediction_by_branch[branch]).strip() != ""
    }
...
def parse_gfa(
    ...
    with gfa_path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if line.startswith("S\t"):
                ...
                seq = parts[2]
                if not node_id:
                    raise ValueError(f"Empty segment ID at {gfa_path}:{line_number}")
                if node_id in segments_seq or node_id in is_primary:
                    raise ValueError(f"Duplicate segment ID at {gfa_path}:{line_number}: {node_id}")
    ...
    for chrom, nodes in backbone_nodes.items():
        ...
        offsets = [offset for offset, _ in nodes]
        if len(offsets) != len(set(offsets)):
            raise ValueError(f"Duplicate backbone offsets found on chromosome '{chrom}'.")
...
def stable_unique_edges(edges: Iterable[str]) -> List[str]:
    """Return deterministic outgoing edges without duplicate link records."""
    return sorted(set(edges), key=natural_sort_key)


def traverse_predicted_path(
    start_node: str,
    chrom: str,
    ...
    """Traverse one predicted alternative and return sequence nodes plus anchor index.

    A path is accepted only when it reaches a downstream primary node on the same
    chromosome. Cycles, cross-chromosome re-entry, and missing segment sequences
    invalidate the candidate so the caller can retain the backbone sequence.
    """
    path_nodes: List[str] = []
    visited: Set[str] = set()
    alt_curr = start_node

    for _ in range(max_alt_steps):
        if alt_curr in visited:
            return [], -1
        visited.add(alt_curr)
        if alt_curr not in segments_seq:
            return [], -1
        path_nodes.append(alt_curr)

        next_edges = stable_unique_edges(adj_list.get(alt_curr, []))
        if not next_edges:
            return [], -1

        same_chrom_hits: List[Tuple[int, str]] = []
        for node_id in next_edges:
            if node_id not in is_primary:
                continue
            hit_pos = node_to_backbone_pos.get(node_id)
            if hit_pos is not None and hit_pos[0] == chrom:
                same_chrom_hits.append((hit_pos[1], node_id))
        if same_chrom_hits:
            same_chrom_hits.sort(key=lambda item: item[0])
            return path_nodes, same_chrom_hits[0][0]

        predicted_next = [
            node_id
            for node_id in next_edges
            if node_id in predicted_nodes and node_id not in is_primary
        ]
        if predicted_next:
            alt_curr = predicted_next[0]
            continue

        # Connector nodes are allowed, but traversal through an unrelated primary
        # node or an unpredicted dead-end must not create a cross-chromosome jump.
        connector_next = [node_id for node_id in next_edges if node_id not in is_primary]
        if len(connector_next) == 1:
            alt_curr = connector_next[0]
            continue
        return [], -1
...
def write_reconstruction_atomically(
    output_file: Path,
    ordered_backbone: Dict[str, List[str]],
    output_chrom_names: Dict[str, str],
    segments_seq: Dict[str, str],
    is_primary: Set[str],
    adj_list: DefaultDict[str, List[str]],
    predicted_nodes: Set[str],
    ...
    """Write reconstructed FASTA to a temporary file and atomically replace output."""
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            for chrom in sorted_chroms:
                ...
                while i < len(bb_list):
                    ...
                    if curr_bb_node not in segments_seq:
                        raise ValueError(
                            f"Primary backbone segment '{curr_bb_node}' has no sequence."
                        )
                    chrom_seq_pieces.append(segments_seq[curr_bb_node])

                    next_edges = stable_unique_edges(adj_list.get(curr_bb_node, []))
                    candidate_alts = [
                        node_id
                        for node_id in next_edges
                        if node_id in predicted_nodes and node_id not in is_primary
                    ...
                    accepted = False
                    for chosen_alt in candidate_alts:
                        temp_alt_seqs, merge_idx = traverse_predicted_path(
                            chosen_alt,
                            chrom,
                            predicted_nodes,
                            is_primary,
                        ...
                        skipped_bp = sum(
                            len(segments_seq[bb_list[k]])
                            for k in range(i + 1, merge_idx)
                        )
                        if skipped_bp > max_skip_bp:
                            continue

                        chrom_seq_pieces.extend(segments_seq[node] for node in temp_alt_seqs)
                        i = merge_idx
                        sv_count += 1
                        accepted = True
                        break
                ...
                print(
                    f"Assembled {chrom} as {output_chrom_names[chrom]}: "
                    f"length={chrom_length:,} bp, inserted_SV={sv_count:,}, "
                    f"intercepted_jumps={intercept_count:,}"
                )
        ...
        if temp_name is not None:
            try:
                ...
                pass
    return total_sv_count, total_intercept_count


def reconstruct_anchored_genome(
    gfa_path: str,
    pred_csv_path: str,
    confidence_csv_path: Optional[str],
    ...
    predicted_nodes, resolved_sample = load_predicted_nodes(
        pred_file,
        target_sample,
    )

    if confidence_csv_path is not None:
        confidence_file = validate_input_file(
            confidence_csv_path,
            "Confidence CSV",
        )

        retained_branches = load_prediction_confidence(
            confidence_file,
            resolved_sample,
            confidence_threshold,
        )
        predicted_nodes = filter_predicted_nodes_by_branches(
            pred_file,
            resolved_sample,
            retained_branches,
        )
    ...
    print(
        f"Loaded predicted nodes after confidence filtering: "
        f"{len(predicted_nodes):,}"
    )
    ...
    total_sv_count, total_intercept_count = write_reconstruction_atomically(
        output_file=output_file,
        ordered_backbone=ordered_backbone,
        output_chrom_names=output_chrom_names,
        segments_seq=segments_seq,
        is_primary=is_primary,
        adj_list=adj_list,
        predicted_nodes=predicted_nodes,
        node_to_backbone_pos=node_to_backbone_pos,
        max_skip_bp=max_skip_bp,
        max_alt_steps=max_alt_steps,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional CSV file containing branch prediction confidence scores."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Minimum confidence required to retain a predicted branch node."
        ),
    )
    parser.add_argument(
        "-s",
...
def main() -> None:
    ...
    reconstruct_anchored_genome(
        ...
        confidence_csv_path=args.confidence,
