...
def load_csv_with_index(path: Path, description: str) -> pd.DataFrame:
    ...
    if dataframe.index.has_duplicates:
        duplicated = dataframe.index[dataframe.index.duplicated()].astype(str).unique().tolist()
        raise ValueError(f"{description} contains duplicated sample IDs: {', '.join(duplicated[:10])}")
    if dataframe.columns.duplicated().any():
        duplicated = dataframe.columns[dataframe.columns.duplicated()].astype(str).unique().tolist()
        raise ValueError(f"{description} contains duplicated columns: {', '.join(duplicated[:10])}")
    return dataframe
...
def align_labels_to_features(features_df: pd.DataFrame, labels_df: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_in_labels = sorted(set(features_df.index).difference(labels_df.index))
    missing_in_features = sorted(set(labels_df.index).difference(features_df.index))
    raise ValueError(
        "Feature and label sample IDs do not match. "
        f"Missing in labels: {', '.join(map(str, missing_in_labels[:5])) or 'none'}; "
        f"missing in features: {', '.join(map(str, missing_in_features[:5])) or 'none'}."
    )
...
def encode_branch_labels(
    ...
    for branch_idx, column in enumerate(labels_df.columns):
        ...
        for node_value, class_idx in value_to_index.items():
            ...
            if node_key in node_to_flat_idx:
                raise ValueError(
                    "A branch-node ID appears in multiple label columns, which makes "
                    f"the relation file ambiguous: {node_key}"
                )
...
def build_laplacian(
    ...
    try:
        ...
        raise ValueError(f"Failed to read relation CSV: {relations_path}") from exc
...
def load_and_preprocess(
    ...
    numeric_features = features_df.apply(pd.to_numeric, errors="coerce")
    x_raw = numeric_features.to_numpy(dtype=np.float32)
...
def save_model(model: nn.Module, output_path: str | Path | None = None) -> Path:
    ...
    model_path = build_default_model_path() if output_path is None else Path(output_path).expanduser()
...
def train_model(args: argparse.Namespace) -> Path:
    ...
    # The caller must provide the source-level training partition only. This
    # prevents this script from silently mixing training, validation, or test data.
    train_dataset = PangenomeDataset(x, y)
