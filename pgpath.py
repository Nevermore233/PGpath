...
def validate_run_arguments(args: argparse.Namespace) -> None:
    """Validate numeric and filesystem inputs before generating any output."""
    input_dir = Path(args.input_dir).expanduser().resolve()
    if not input_dir.is_dir():
        raise NotADirectoryError(f"Input FASTQ directory does not exist: {input_dir}")
    args.input_dir = str(input_dir)
    if not str(args.population_name).strip():
        raise ValueError("population_name must not be empty.")
    if args.threads <= 0:
...
def run_pipeline(args: argparse.Namespace) -> Path:
    ...
    if fasta_output.exists() and fasta_output.is_dir():
        raise IsADirectoryError(f"Output FASTA path points to a directory: {fasta_output}")
    ...
    try:
        ...
        generated_intermediates.append(Path(population_feature_csv))
        ...
        generated_intermediates.append(Path(prediction_csv))
        ...
        if not args.keep_intermediate:
            for path in generated_intermediates:
                try:
